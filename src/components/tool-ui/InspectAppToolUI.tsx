...
const InspectAppToolCard = ({
  args,
  result,
  status,
  isError,
  addResult,
}: ToolCallMessagePartProps<InspectAppToolArgs, InspectAppToolResult>) => {
  ...
  const title = args?.title ?? "App inspection";
  const prompt =
    args?.prompt ?? `Inspecting ${args?.url ?? "target URL"} with Stage1.`;
  const url = args?.url ?? "";
  ...
  const runInspection = useCallback(async () => {
    ...
    try {
      ...
      const inspectionResult = await client.inspectApp({
        url,
        name: args?.name,
  ...
  return (
    <ToolOutputCard>
      <ToolOutputCardHeader>
        <ToolOutputCardHeading>
          <ToolOutputCardEyebrow>Stage1 Inspect App</ToolOutputCardEyebrow>
      ...
      <ToolOutputCardBody>
        <ToolOutputCardMeta>
          <div className="space-y-1">
            <div>URL: {url || "—"}</div>
            {args?.crawlDepth != null && (
              <div>Crawl depth: {args.crawlDepth}</div>
            )}
            {args?.include?.length ? (
              <div>Include: {args.include.join(", ")}</div>
            ) : null}
            {args?.components && <div>Component analysis: enabled</div>}
        ...
        {phase === "inspecting" && (
          <ToolOutputCardCallout tone="warning">
            <div className="flex items-center gap-2">
              ...
              <span>
                Inspecting {url}... ({formatElapsed(elapsedSeconds)})
              </span>
            </div>
            <div className="mt-1 text-xs opacity-70">
              This may take 1–5 minutes depending on site complexity.
            </div>
          </ToolOutputCardCallout>
        )}
          ...
          <ToolOutputCardCallout tone="success" className="space-y-2">
            <div className="text-xs uppercase tracking-[0.2em] text-emerald-100/70">
              {discovery?.bundleLoaded
                ? "Inspection & bundle loaded"
                : "Inspection complete"}
            </div>
            <div className="text-sm font-medium text-emerald-50">
              {result.hostname ?? url} inspected successfully.
              {discovery?.bundleLoaded && (
                <>
                  {" "}
...
export const InspectAppToolUI = makeAssistantToolUI<
  InspectAppToolArgs,
  InspectAppToolResult
>({
  toolName: INSPECT_APP_TOOL_NAME,
  render: InspectAppToolCard,
});
