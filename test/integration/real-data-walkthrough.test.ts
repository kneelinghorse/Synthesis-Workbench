...
describe("Real-Data Pipeline Walkthrough (s5-m06)", () => {
  ...
  describe("Step 5 — Full Pipeline: Ingest → Seed → Compose → Render → Export", () => {
    it("runs the complete pipeline and exports valid HTML", async () => {
      const client = createRealDataClient();
      (getFoundryMcpClient as ReturnType<typeof vi.fn>).mockReturnValue(
        client
      );

      // 1. Ingest real Stage1 bundle
      const bundleResult =
        useStage1BundleStore.getState().loadBundle(REAL_STAGE1_BUNDLE);
      expect(bundleResult.ok).toBe(true);

      // 2. Seed tokens from real token-guess
      const seedResult = useStage1BundleStore.getState().seedTokenState();
      expect(seedResult.appliedCount).toBeGreaterThan(0);
      ...
      const composition = await renderDocument(REAL_DATA_DOCUMENT, client);
      expect(composition.errors).toHaveLength(0);
      usePreviewStateStore.getState().setHtml(composition.html);

      // 5a. Export HTML — validates token CSS variables and rendered content
      const htmlExport = executeExportDesign({
        requestId: "export-html",
        format: "html",
        slug: "walkthrough",
      });
      ...
      expect(htmlExport.content).toContain(
        "<title>Stage1 Walkthrough — Contact Form</title>"
      );
      // Real token values in CSS variables
      expect(htmlExport.content).toContain("--colors-primary: #3b82f6");
      expect(htmlExport.content).toContain("--colors-secondary: #64748b");
      expect(htmlExport.content).toContain("--colors-accent: #f59e0b");
      expect(htmlExport.content).toContain("--spacing-md: 1rem");
      expect(htmlExport.content).toContain("--radius-md: 0.375rem");
      // Font family from real token-guess
      expect(htmlExport.content).toContain("Inter, system-ui, sans-serif");
      // Rendered component content
      expect(htmlExport.content).toContain("NavLink");
      expect(htmlExport.content).toContain("Button");
    });

    it("runs the complete pipeline and exports valid JSON", async () => {
      const client = createRealDataClient();
      (getFoundryMcpClient as ReturnType<typeof vi.fn>).mockReturnValue(
        client
      );

      // 1-4: Ingest → Seed → Compose → Render
      useStage1BundleStore.getState().loadBundle(REAL_STAGE1_BUNDLE);
      useStage1BundleStore.getState().seedTokenState();
      await executeSetDocument({
        requestId: "json-pipeline",
        document: REAL_DATA_DOCUMENT,
        slug: "walkthrough",
      });
      const composition = await renderDocument(REAL_DATA_DOCUMENT, client);
      ...
      expect(parsed.document.metadata.title).toBe(
        "Stage1 Walkthrough — Contact Form"
      );
      expect(parsed.document.root.nodeType).toBe("layout");

      // Token state contains real values
      expect(parsed.tokenState.colors.primary).toBe("#3b82f6");
      expect(parsed.tokenState.colors.secondary).toBe("#64748b");
      expect(parsed.tokenState.typography.fontFamily.sans).toBe(
        "Inter, system-ui, sans-serif"
      );
      expect(parsed.tokenState.spacing.md).toBe("1rem");
      expect(parsed.tokenState.radius.md).toBe("0.375rem");
    ...
    it("runs the complete pipeline and exports valid YAML", async () => {
      const client = createRealDataClient();
      (getFoundryMcpClient as ReturnType<typeof vi.fn>).mockReturnValue(
        client
      );
